...
namespace dxvk {
  ...
  void Presenter::updateDisplayTiming() {
    ...
    const char* isVariableRefresh = info.isVariableRefresh ? "(variable refresh)" : "(fixed refresh)";

    if (info.refreshIntervalNs) {
      auto refreshRateToLog = 10000000000ull / info.refreshIntervalNs;
      Logger::info(str::format("Presenter: Reported refresh rate: ",
          (refreshRateToLog / 10u), ".", (refreshRateToLog % 10u), " Hz ", isVariableRefresh));
    }
  }
  ...
  void Presenter::runFrameThread() {
    ...
    while (true) {
      ...
      m_frameCond.wait(lock, [this] {
        return m_frameQueuePushId > m_frameQueuePopId && m_frameQueue[m_frameQueuePopId % m_frameQueue.size()].frameId <= m_lastSignaled;
      });

      // Use a frame ID of 0 as an exit condition
      PresenterFrame frame = m_frameQueue[m_frameQueuePopId % m_frameQueue.size()];
