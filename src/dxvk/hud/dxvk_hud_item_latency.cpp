...
namespace dxvk::hud {
  ...
  void HudLatencyDetailsItem::update(dxvk::high_resolution_clock::time_point time) {
    ...
    if (elapsed.count() >= UpdateInterval) {
      ...
      bool presentTiming = framePacer->m_device->features().extPresentTiming.presentTiming;

      if (framePacer->getFramePacerMode()->getPresentMode() == VK_PRESENT_MODE_FIFO_KHR
        && (presentTiming || (framePacer->getMode() || std::chrono::duration_cast<std::chrono::milliseconds>(
